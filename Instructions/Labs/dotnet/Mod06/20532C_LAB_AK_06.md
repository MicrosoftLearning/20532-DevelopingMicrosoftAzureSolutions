...
3.  In the **Libraries** window, go to **Allfiles (F):\\Mod06\\Labfiles\\Starter\\Contoso.Events**, and then double-click **Contoso.Events.sln.**
...
3.  At the end of the **GetRegistrantsNames** method and before the closing parenthesis, create a new instance of the **TableQuery** class and use the fluent **Where** method with your filter to generate a query:
...
10. Record the *Key* value listed for the access key you selected.

11. For the access key you selected, click the three ellipsis (...) button to the right of the key. Once clicked, select the **View connection string** option.
...
1.  In the **Server Explorer** pane, locate and click the arrow at the left side of the **Azure** node to expand the node.

2.  Locate the **Storage** node and click the arrow at the left side to expand the node.

4.  If prompted for your account credentials, sign in by using your **Microsoft Account**.

4.  Right-click the **Storage** node and select the **Attach External Storage** option.

5.  In the **Add New Storage Account** dialog, perform the following actions:

    a. Enter the **Account name** and **Account key** from your connection information recorded earlier in this lab.

    b. Ensure that the **Remember account key** option is selected.

    c. Ensure that the **Use HTTPS (Recommended)** option is selected in the *Connection* section.

    d. Ensure that the value in the **Preview connection string** section matches the *connection string* you recorded earlier.

    d. Click the **OK** button.

5.  In the **Server Explorer** pane, expand the node for your newly added *Storage Account* under the **Storage Accounts** node.
...
2.  Locate the **Storage** node under the **Azure** node and click the arrow at the left side.
